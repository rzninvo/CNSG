...
def summarise_frames(frames: Sequence[Dict[str, Any]]) -> List[FrameSummary]:
    ...
    for frame in frames:
        ...
        relation_data = frame.get("spatial_relations") or frame.get("relationships") or []
...
def build_prompt(scene_index: str | None, summaries: Sequence[FrameSummary]) -> str:
    intro_lines = [
        ...
        "Prioritise landmarks at a comfortable mid-range distance (roughly 3-5 meters) when describing guidance.",
        "Treat very close (<1m) or distant (>6m) items as secondary context unless they are critical to direction.",
        "Craft a concise description (under 180 words) that guides a listener along the path.",
        "Base every detail on the provided observations and landmarks; do not invent objects.",
        "Highlight transitions and prominent landmarks that would help someone retrace the path.",
    ]

    header = "\n".join(intro_lines)
    scene_line = f"Scene index: {scene_index}" if scene_index else "Scene index: unknown"
    observation_lines = "\n".join(f"- {summary.to_prompt_line()}" for summary in summaries)
