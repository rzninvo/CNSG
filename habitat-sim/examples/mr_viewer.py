...
class HabitatSimInteractiveViewer(Application):
    ...
    def shortest_path(self, sim, goal: mn.Vector3):  # TODO REMOVE ALL THE PRINTS
        if not sim.pathfinder.is_loaded:
            ...
            # fmt on
            agent_state_pos = mn.Vector3(
                self.sim.get_agent(self.agent_id).get_state().position
            )
    ...
    def get_object_position(
        ...
        for region in scene.regions:
            ...
            region_pos = mn.Vector3(
                region_meta.get("position", (-1, -1, -1))
            )
    ...
    while True:
        try:
            ...
            if user_input:
                ...
                if goal_pos is None:
                    print(f"Warning: '{target_name}' not found in the scene.")
                    continue
